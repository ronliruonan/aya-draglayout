...
独立的 Vue 3 + TypeScript 桌面布局编辑应用。支持旧版样例提示、计时器、CMD 加载，以及文本、指标卡、容器、拖拽与排序、属性编辑、预览、JSON 导入导出、撤销重做。已支持本地模拟生成闭环，尚未接入真实 AI。
...
组件区仅展示内容挂件。画布工具栏提供“添加布局”的单列、双列、三列入口；选中布局区域后可修改标题与列数。JSON 中仍使用 container 类型，已有页面无需迁移。

选中布局后，右侧“添加子布局”可直接创建单列、双列或三列嵌套布局，新子布局会自动选中。顶部入口始终添加到最外层；所有新增操作可撤销。

## 模拟生成布局

点击“生成布局（模拟）”，输入列数和挂件关键词，再生成候选布局。当前使用本地规则，支持单/双/三列、文本、指标、计时、加载、样例，不发送请求或使用 API Key。

生成结果先通过页面协议校验并预览。点击“确认应用”才通过 core replace 替换整个页面，支持撤销恢复；关闭、取消或生成失败不会修改当前页面。修改描述后必须重新生成。真实 OrcaRouter Provider 尚未接入。
